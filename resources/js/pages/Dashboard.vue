...
interface DissidioSummary {
    ano: number;
    percentual: number;
    status: string;
    label: string;
}
interface PlrSummary {
    ano: number;
    status: string;
    label: string;
    valor_total: number | null;
}
interface ThirteenthSummary {
    ano: number;
    status: string;
    label: string;
}
...
const kpis = computed(() => [
    {
        ...
        color: '#0096ca',
        trend:
            props.collaboratorStats.novos_mes > 0
                ? `+${props.collaboratorStats.novos_mes} este mês`
                : 'Sem novos este mês',
        href: '/collaborators',
    },
    {
    ...
    {
        label: props.dissidioSummary
            ? `Dissídio ${props.dissidioSummary.ano}`
            : 'Dissídio',
        value: props.dissidioSummary
            ? `${props.dissidioSummary.percentual}%`
            : '—',
    ...
    },
    {
        ...
        value: props.plrSummary?.valor_total
            ? formatCurrency(props.plrSummary.valor_total)
            : '—',
    ...
    <div class="flex flex-col gap-8 p-6 pb-12">
        ...
        <div class="fade-up fade-up-1 flex items-end justify-between">
            <div>
                ...
                >
                    Folha de Pagamento
                    <span v-if="payrollSummary">
                        · {{ payrollSummary.mes_referencia }}</span
                    >
                </p>
                <h1
                    class="gradient-text stat-number font-display text-5xl font-extrabold"
                >
                    {{
                        payrollSummary
                            ? formatCurrency(payrollSummary.liquido)
                            : 'Nenhum ciclo aberto'
                    }}
                </h1>
            ...
            <div class="flex items-center gap-3">
                ...
                <div
                    ...
                    :style="{
                        background: `${payrollStatusColor}14`,
                        borderColor: `${payrollStatusColor}33`,
                        color: payrollStatusColor,
                    }"
                >
                    <span
                        class="pulse-dot inline-block h-1.5 w-1.5 rounded-full"
                        :style="{ background: payrollStatusColor }"
                    />
                    {{ payrollSummary.status_label }}
                </div>
            </div>
        ...
        <div class="grid gap-6 lg:grid-cols-[1fr_360px]">
            ...
            <div class="fade-up fade-up-7 glass-card p-5">
                <div class="mb-4 flex items-center justify-between">
                    <h2
                        class="font-display text-sm font-bold tracking-wider text-foreground uppercase"
                    >
                        Atividade Recente
                    </h2>
                ...
                >
                    ...
                    >
                        <div
                            class="mt-1 flex size-6 shrink-0 items-center justify-center rounded-full"
                            :style="{
                                background:
                                    item.tipo === 'success'
                                        ? 'rgba(16,185,129,0.1)'
            ...
            <Link
                href="/dp-assistant"
                class="fade-up fade-up-8 glass-card group flex flex-col gap-4 p-5 transition-all hover:opacity-90"
            >
                <div class="flex items-center gap-3">
                    <div
                    ...
                    <div>
                        ...
                        <p class="text-xs text-muted-foreground">
                            Pergunte sobre folha, férias, PLR...
                        </p>
                    </div>
                </div>
                <p class="text-xs text-muted-foreground">
                    IA treinada nas regras trabalhistas brasileiras com acesso
                    aos dados reais do sistema.
                </p>
                <div class="flex flex-wrap gap-2">
                    <span
                        v-for="chip in [
                            'Elegibilidade férias',
                            'Cálculo 13°',
                            'DSR comissões',
                        ]"
                        :key="chip"
                        ...
                        style="
                            ...
                            color: #0096ca;
                            border: 1px solid rgba(0, 150, 202, 0.2);
                        "
                    >
                        {{ chip }}
                    </span>
