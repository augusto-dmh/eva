...
import { index, update } from '@/routes/collaborators';
import type {
    Collaborator,
    EnumOption,
    LegalEntity,
...
defineOptions({
    layout: {
        breadcrumbs: [
            ...
            { title: 'Editar Colaborador', href: '#' },
        ],
    },
});

const form = useForm({
    nome_completo: props.collaborator.nome_completo,
    cpf: props.collaborator.cpf,
    email_corporativo: props.collaborator.email_corporativo,
    email_pessoal: props.collaborator.email_pessoal ?? '',
    data_nascimento: props.collaborator.data_nascimento ?? '',
    telefone: props.collaborator.telefone ?? '',
    tipo_contrato: props.collaborator.tipo_contrato,
    legal_entity_id: String(props.collaborator.legal_entity_id),
    departamento: props.collaborator.departamento ?? '',
    cargo: props.collaborator.cargo ?? '',
    nivel: props.collaborator.nivel ?? '',
    trilha_carreira: props.collaborator.trilha_carreira ?? '',
    lider_direto: props.collaborator.lider_direto ?? '',
    status: props.collaborator.status,
    data_admissao: props.collaborator.data_admissao,
    data_desligamento: props.collaborator.data_desligamento ?? '',
    flash_numero_cartao: props.collaborator.flash_numero_cartao ?? '',
    flash_vale_alimentacao: props.collaborator.flash_vale_alimentacao ?? '',
    flash_vale_refeicao: props.collaborator.flash_vale_refeicao ?? '',
    flash_vale_transporte: props.collaborator.flash_vale_transporte ?? '',
    flash_saude: props.collaborator.flash_saude ?? '',
    flash_cultura: props.collaborator.flash_cultura ?? '',
    flash_educacao: props.collaborator.flash_educacao ?? '',
    flash_home_office: props.collaborator.flash_home_office ?? '',
    salario_base: props.collaborator.salario_base,
    tipo_comissao: props.collaborator.tipo_comissao,
    minimo_garantido: props.collaborator.minimo_garantido ?? '',
    elegivel_comissao: props.collaborator.elegivel_comissao,
    desconto_petlove: props.collaborator.desconto_petlove ?? '',
    banco: props.collaborator.banco ?? '',
    agencia: props.collaborator.agencia ?? '',
    conta: props.collaborator.conta ?? '',
    chave_pix: props.collaborator.chave_pix ?? '',
    pis: props.collaborator.pis ?? '',
    slack_user_id: props.collaborator.slack_user_id ?? '',
});

function submit() {
    form.put(update(props.collaborator).url);
}
</script>
